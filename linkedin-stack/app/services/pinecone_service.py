...
async def get_context_for_auto_post(user_id: str, supabase: Client) -> str:
    ...
    try:
        # This part remains the same: we use the full profile to create the query vector
        ...
        # This part is new: fetch the specific topic for filtering
        logging.info(f"[{user_id}] Fetching brand profile topic from Supabase for filtering.")
        brand_topic = await supabase_service.get_brand_profile_topic(user_id, supabase)
        ...
        logging.info(f"[{user_id}] Querying Pinecone for auto-post context with topic filter: '{brand_topic}'.")
        query_response = pc_index.query(
            vector=query_embedding,
            top_k=5,
            namespace=user_id,
            filter={"topic": brand_topic}, # Use the correct filter
            include_metadata=True
        )
        logging.info(f"[{user_id}] Pinecone query successful. Found {len(query_response['matches'])} matches.")
        
        if not query_response['matches']:
            logging.warning(f"[{user_id}] No context found in Pinecone for auto-post with topic filter: '{brand_topic}'.")
            return ""
...
async def get_context_for_manual_post(user_id: str, topic: str) -> str:
    """
    ...
    try:
        ...
        query_response = pc_index.query(
            ...
            top_k=5,
            namespace=user_id,
            # No filter is applied, searching all data for the user.
            include_metadata=True
        )
